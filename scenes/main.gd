...
func _ready() -> void:
	GameDataGlobal.on_game_success.connect(_game_success)
	restart_game()
...
func restart_game():
	...
	GameDataGlobal.WIN_GAME_SCORE = int(GameDataGlobal.MAX_SCORE * 0.4 / 100) * 100
